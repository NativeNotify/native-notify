...
**The fix (this commit):** four guards in the listener — (1) skip when the inbound token equals the last posted device token (no-op self-fires), (2) throttle to ≤1 check/minute, (3) `rotationInFlight` overlap guard, (4) only re-post when the token payload actually changed — a no-op self-fire is skipped without even a fetch.

**Status (evening of 2026-09-16):** **PUBLISHED to npm** — `native-notify@5.1.1` is `latest` (published by the user from their terminal; account requires npm's web-OTP flow that a non-interactive shell can't complete). Verified: registry version 5.1.1, `gitHead` = `c6aeb95`, and the downloaded tarball contains the guard code (`TOKEN_ROTATION_MIN_INTERVAL_MS`, `lastRotationHandledAt`) plus the 5.1 analytics module. WayChat bumped its dependency to 5.1.1 in commit `9688338`; installed apps get the fix with their next build.